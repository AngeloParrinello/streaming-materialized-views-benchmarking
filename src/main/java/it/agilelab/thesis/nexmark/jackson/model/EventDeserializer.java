...
public class EventDeserializer extends JsonDeserializer<Event<?>> {
     ...
     */
    @Override
    public Event<?> deserialize(final JsonParser p,
                                final DeserializationContext ctx) throws IOException {
        JsonNode jsonNode = p.readValueAsTree();

        if (jsonNode.isObject()) {
            JsonNode eventNode = jsonNode.get("actualEvent");
            if (eventNode != null) {
                Class<?> type = selectTypeToSerialize(jsonNode);
                Object element = JacksonUtils.getElementAs(jsonNode, "actualEvent", type);
                return new Event<>(element);
            }
        }

        throw new IOException("Not a valid event: " + jsonNode);
    }

    private Class<?> selectTypeToSerialize(final JsonNode objectNode) throws IOException {
        String type = objectNode.get("eventType").asText();
        switch (type) {
            case "AUCTION":
            ...
            case "PERSON":
                return Person.class;
            default:
                throw new IOException("Not a valid event: " + objectNode);
        }
    }
