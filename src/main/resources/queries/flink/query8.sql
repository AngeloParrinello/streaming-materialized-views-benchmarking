...
FROM (SELECT id,
      ...
      FROM TABLE(TUMBLE(TABLE people, DESCRIPTOR(entryTime), INTERVAL '10' SECOND))
      GROUP BY id, name, window_start, window_end) P
         JOIN (SELECT seller,
                      window_start AS starttime,
                      window_end   AS endtime
               -- creates a tumbling window of 10 seconds that holds the auctions that have been created in the last 10 seconds
               FROM TABLE(TUMBLE(TABLE auctions, DESCRIPTOR(entryTime), INTERVAL '10' SECOND))
